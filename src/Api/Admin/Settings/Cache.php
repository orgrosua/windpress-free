...
class Cache extends AbstractApi implements ApiInterface
{
    ...
    public function store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        try {
            $content = sprintf("/*! %s v%s | %s | %s */\n%s", strtolower(Common::plugin_data('Name')), WIND_PRESS::VERSION, date('Y-m-d H:i:s', time()), strtolower(Common::plugin_data('PluginURI')), base64_decode($payload['content']));
    ...
    public function providers_scan(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        return new WP_REST_Response(['metadata' => ['provider_id' => $provider['id'], 'provider_name' => $provider['name'], 'profiling' => ['unix_timestamp' => time(), 'duration' => $event->getDuration() . ' ms', 'memory' => sprintf('%.2F MiB', $event->getMemory() / 1024 / 1024)], 'next_batch' => $result['metadata']['next_batch'] ?? \false], 'contents' => $result['contents'], 'status' => 'OK']);
    }
}
